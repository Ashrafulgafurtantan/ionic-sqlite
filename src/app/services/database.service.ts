...
export class DatabaseService {
  ...
  loadProducts() {
    const query = 'SELECT product.name, product.id, ' +
      'developer.name AS creator FROM product JOIN developer ON developer.id = product.creatorId';
  ...
  updateDeveloper(dev: Dev) {
    ...
    return this.database.executeSql(`UPDATE developer
                                     SET name   = ?,
                                         skills = ?,
                                         img    = ?
                                     WHERE id = ${dev.id}`, data).then(() => {
      this.loadDevelopers();
    });
  }
